...
This document records exact-commit evidence for release `v0.2.2` of Agent Browser. Runtime claims
are bound to one runtime commit, one immutable image ID, and one hosted workflow run.

Tested runtime commit: `648269bf4ec5da534f2a4afe81104ab52c826bb3`

Release: `v0.2.2`, source-only. The tag is created from the manifest-closure commit after this
...
adds the MCP Registry publication surface — a root `server.json`, an `mcpName` field in the npm
client, and an `mcp-name` marker in the PyPI client's long description — and changes no runtime
behaviour; the API contract stays `api_version: "v1"`.
...
[run 33865946626](https://github.com/AetherAI3/agent-browser/actions/runs/33865946626).

| Gate | Required evidence | Recorded result |
|---|---|---|
| Clean checkout | exact commit and clean status | `ref-proof` resolved `main` to `648269bf…`; every downstream job checked out and re-verified that exact SHA ([job](https://github.com/AetherAI3/agent-browser/actions/runs/33865946626/job/101000720393)) |
| Hosted CI | exact-commit run URL and job matrix | `ci` succeeded at `648269bf…`: [run 33865946632](https://github.com/AetherAI3/agent-browser/actions/runs/33865946632) |
...
| SBOM and vulnerability scan | artifact names and SHA-256 | Syft `1.51.1` → `sbom.spdx.json` (`de35b2061624262c65c31ff253e6b8f1a02543b8d5ed017d6454b3d0b9442fc5`, 630 packages); Grype `0.118.0` → `vulnerabilities.json` (`1b6777d1db80596aace4d2bef58e778de5a80511a3e4a03d1d311906ee20d498`) |
| Rootless image build | image ID and base digest | Image ID `sha256:4df5d32466c633927410e5a437f6574bbb12d7c29c5066b47faf22856f62faa2`; base `docker.io/library/python@sha256:528257d48c1da0dcecc2e725d1ae34498d60c965f1241e39cd6a85a8859bdf84`; final image runs as `USER 10001:10001` ([job](https://github.com/AetherAI3/agent-browser/actions/runs/33865946626/job/101000764793)) |
| Isolated acceptance | exact image ID, network proof, cleanup proof | `acceptance.json` = PASS over 33 checks against the same image ID, including `isolated-pod-network-none`, `same-pod-namespace`, `loopback-listener-proof`, `same-display-color-proof`, `process-cleanup`, `profile-cleanup`, and `pod-cleanup` ([job](https://github.com/AetherAI3/agent-browser/actions/runs/33865946626/job/101002036183)) |
| README quickstart | fresh-directory reproduction | `quickstart.json` = PASS for `docker compose up --build --detach` at the same commit ([job](https://github.com/AetherAI3/agent-browser/actions/runs/33865946626/job/101000764847)) |
...
`version-agreement` (`0.2.2` across project, package, Compose, and image label), `secret-scan`,
...
- **Runtime commit** `648269bf4ec5da534f2a4afe81104ab52c826bb3` is the reviewed code under test.
...
- **Manifest-closure commit** adds that generated manifest verbatim and is the commit tagged
  `v0.2.2`. The embedded manifest attests its evidence-commit parent. A fresh successful run at the
...
claim that the image is free of vulnerabilities. `vulnerabilities.json` reports an empty
`matches` set and 917 entries under `ignoredMatches`, suppressed by the recorded policy: 12 carry
an available fix, 570 have no released fix, 330 are marked will-not-fix by the Debian-derived base,
and 5 report no fix state. By severity: 42 critical, 198 high, 289 medium, 57 low, 290 negligible,
and 41 unknown. Every figure here was re-counted from this run's own `vulnerabilities.json` rather
than carried over, and it matches `0.2.1` exactly because the base image digest, the hash-locked
Python environment, and the Chrome package version are all pinned and did not move between the two
releases. This is a scan-time snapshot against the workflow's refreshed vulnerability database;
...
- Runtime, authority, navigation, container, CI, package, and public-version surfaces bind to
  `648269bf4ec5da534f2a4afe81104ab52c826bb3`.
