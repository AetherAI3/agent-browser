...
Tested runtime commit: `873b59a3ad63d4d828ec08069fbe1b96380b186b`
...
[run 33676078669](https://github.com/AetherAI3/agent-browser/actions/runs/33676078669).

| Gate | Required evidence | Recorded result |
|---|---|---|
| Clean checkout | exact commit and clean status | `ref-proof` resolved `main` to `873b59a3…`; every downstream job checked out and re-verified that exact SHA ([job](https://github.com/AetherAI3/agent-browser/actions/runs/33676078669/job/100400968252)) |
| Hosted CI | exact-commit run URL and job matrix | `ci` succeeded at `873b59a3…`: [run 33676078397](https://github.com/AetherAI3/agent-browser/actions/runs/33676078397) |
...
| SBOM and vulnerability scan | artifact names and SHA-256 | Syft `1.51.1` → `sbom.spdx.json` (`62974b8f9740f5f7ecce918aa80d2410e0fea20216bcc0c7132eb613936e4faa`, 629 packages); Grype `0.118.0` → `vulnerabilities.json` (`527a4c59aa1e282ba453b15140888203bea20417dea323c80a0215aefa24fcc3`) |
| Rootless image build | image ID and base digest | Image ID `sha256:cedb62d6e9cd9e3f7e93d5d5a5076b2f9e57e48c3492f472ac05ea8546201366`; base `docker.io/library/python@sha256:528257d48c1da0dcecc2e725d1ae34498d60c965f1241e39cd6a85a8859bdf84`; final image runs as `USER 10001:10001` ([job](https://github.com/AetherAI3/agent-browser/actions/runs/33676078669/job/100401014851)) |
| Isolated acceptance | exact image ID, network proof, cleanup proof | `acceptance.json` = PASS over 33 checks against the same image ID, including `isolated-pod-network-none`, `same-pod-namespace`, `loopback-listener-proof`, `process-cleanup`, `profile-cleanup`, and `pod-cleanup` ([job](https://github.com/AetherAI3/agent-browser/actions/runs/33676078669/job/100402765846)) |
| README quickstart | fresh-directory reproduction | `quickstart.json` = PASS for `docker compose up --build --detach` at the same commit ([job](https://github.com/AetherAI3/agent-browser/actions/runs/33676078669/job/100401014682)) |
...
- **Runtime commit** `873b59a3ad63d4d828ec08069fbe1b96380b186b` is the code under test. Every
...
- Runtime, authority, navigation, container, CI, and documentation all refer to
  `873b59a3ad63d4d828ec08069fbe1b96380b186b`.
